...
Fixed:
  - log_audit on failure now passes AuditStatus.FAILED (not an integer)
  - Admin included in role guards
"""
...
async def generate_diagnostic_report(
    ...
    Orchestrates:
      1. BioBERT — clinical note text embedding
      2. BiomedCLIP — imaging embedding
      3. FAISS — vector similarity retrieval
      4. Neo4j — SNOMED CT knowledge graph query
      5. Groq — entity extraction + differential diagnosis (cost-effective)
      6. Gemini — multi-modal final report generation
    """
    if not query_data.patient_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="patient_id is required")

    result = await db.execute(select(Patient).where(Patient.uuid == query_data.patient_id))
    ...
    try:
        ...
        await log_audit(
            db=db, user=current_user, action=AuditAction.GENERATE_DIAGNOSTIC,
            target_type="diagnostic_report", patient_uuid=patient.uuid,
            action_details="Generated AI diagnostic report (Gemini + Groq)", request=request,
        )
        ...
        # Fixed: pass AuditStatus.FAILED (enum), not an integer
        await log_audit(
            db=db, user=current_user, action=AuditAction.GENERATE_DIAGNOSTIC,
            status=AuditStatus.FAILED,                          # ← was: status.HTTP_500_…
            target_type="diagnostic_report", patient_uuid=patient.uuid,
            action_details=f"Failed: {str(e)}", request=request,
        )
        raise HTTPException(
...
async def get_patient_diagnostic_reports(
    ...
    Physicians/Admin can view any patient; patients can only view their own.
    """
    ...
    await log_audit(
        db=db, user=current_user, action=AuditAction.VIEW_DIAGNOSTIC,
...
async def analyze_medical_image(
    ...
    try:
        ...
        return {"image_path": image_path, "analysis": analysis, "model": "gemini-1.5-pro-vision"}
    except Exception as e:
...
async def summarize_clinical_note(
    ...
    try:
        summary = await get_groq_service().summarize_clinical_note(
            clinical_note=note_text, max_length=max_length
        )
        return {
...
async def extract_medical_entities(
    ...
    """Extract medical entities (symptoms, diagnoses, meds, etc.) from text using Groq."""
    from src.services.groq import get_groq_service
    try:
