...
async def create_imaging_study(study_data: ImagingStudyCreate, patient_id: str, request: Request,
                                current_user: User = Depends(require_roles("nurse", "radiologist", "physician", "admin")),
    ...
    study = ImagingStudy(patient_uuid=patient_id, study_date=study_data.study_date, modality=modality,
...
async def list_imaging_studies(request: Request, patient_id: Optional[str] = Query(None),
                                modality: Optional[str] = Query(None), status_filter: Optional[str] = Query(None),
...
async def get_patient_imaging_studies(patient_id: str, request: Request,
                                       current_user: User = Depends(require_roles("nurse", "radiologist", "physician", "admin", "compliance", "patient")),
                                       db: AsyncSession = Depends(get_db)):
    ...
    await log_audit(db=db, user=current_user, action=AuditAction.VIEW_IMAGING,
...
async def get_imaging_study(study_id: str, request: Request,
                             current_user: User = Depends(require_roles("nurse", "radiologist", "physician", "admin", "compliance", "patient")),
                             db: AsyncSession = Depends(get_db)):
...
async def add_interpretation(study_id: str, interpretation: ImagingStudyUpdate, request: Request,
                              current_user: User = Depends(require_roles("radiologist", "admin")),
                              db: AsyncSession = Depends(get_db)):
    """Add radiologist interpretation to an imaging study. Auto-embeds findings for RAG."""
    study = (await db.execute(select(ImagingStudy).where(ImagingStudy.uuid == study_id))).scalars().first()
    if not study:
        raise HTTPException(status_code=404, detail="Imaging study not found")
    ...
    await db.commit()
    await db.refresh(study)
    # Embed findings for RAG retrieval
    if study.findings:
        try:
    ...
    await log_audit(db=db, user=current_user, action=AuditAction.ADD_IMAGING_NOTE,
...
async def update_imaging_study(study_id: str, study_update: ImagingStudyUpdate, request: Request,
                                current_user: User = Depends(require_roles("radiologist", "physician", "admin")),
                                db: AsyncSession = Depends(get_db)):
...
async def delete_imaging_study(study_id: str, request: Request,
