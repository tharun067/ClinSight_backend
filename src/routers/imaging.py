...
Fixed: admin included in all role guards, correct AuditStatus on failure paths.
"""
...
async def create_imaging_study(
    ...
    await log_audit(
        ...
        target_type="imaging_study", target_uuid=imaging_study.uuid,
        patient_uuid=patient_id,
...
async def get_imaging_study(
    ...
    await log_audit(
        ...
        patient_uuid=study.patient_uuid, request=request,
    )
    return study
...
async def update_imaging_study(
    ...
    await log_audit(
        db=db, user=current_user, action=AuditAction.OTHER,
        target_type="imaging_study", target_uuid=study_id,
        patient_uuid=study.patient_uuid, action_details="Updated imaging study", request=request,
    )
    return study
...
async def add_interpretation(
    ...
    """Add radiologist interpretation to an imaging study."""
    study = (await db.execute(select(ImagingStudy).where(ImagingStudy.uuid == study_id))).scalars().first()
    if not study:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Imaging study not found")
    ...
    if study.findings:
        try:
            text_content = (
                f"{study.modality.value} of {study.body_part}. "
                f"Findings: {study.findings}. "
                f"Impression: {study.impression or ''}"
            )
